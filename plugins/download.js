...
*└─────────────────────┘*`
        ...
        // Function to download and send video or document
        async function downloadMedia(quality, type, thumbnail, title) {
            try {
                const apiUrl = `https://tharuzz-ofc-api-v3.vercel.app/api/ytdl/yt?url=${encodeURIComponent(result.url)}&format=${quality}`;
                ...
                if (data.download) {
                    if (type === 'video') {
                        await conn.sendMessage(from, {
                            ...
                            caption: `Downloaded ${quality}p video`,
                            mimetype: 'video/mp4'
                        }, { quoted: mek });
                        ...
                        await conn.sendMessage(from, {
                            ...
                            mimetype: 'video/mp4',
                            fileName: `${title}.mp4`,
                            caption: `Downloaded ${quality}p document`
                        }, { quoted: mek });
                    }
                } else {
                    await reply(`*Failed to get download link for ${quality}p ${type}!*`);
                }
            } catch (e) {
                await reply(`*Error downloading ${type}!*`);
                console.error(e);
            }
        }
        ...
        const sections = [
            {
                title: "`Video type 📽️`",
                rows: [
                    { title: '```144p Video```', rowId: prefix + `video_download ${result.url}|144|video` },
                    { title: '```240p Video```', rowId: prefix + `video_download ${result.url}|240|video` },
                    { title: '```360p Video```', rowId: prefix + `video_download ${result.url}|360|video` },
                    { title: '```720p Video```', rowId: prefix + `video_download ${result.url}|720|video` },
                    { title: '```1080p Video```', rowId: prefix + `video_download ${result.url}|1080|video` }
                ]
            },  
            {
                title: "`Document type 📁`",
                rows: [
                    { title: '```144p Document```', rowId: prefix + `video_download ${result.url}|144|document|${result.thumbnail}|${result.title}` },
                    { title: '```240p Document```', rowId: prefix + `video_download ${result.url}|240|document|${result.thumbnail}|${result.title}` },
                    { title: '```360p Document```', rowId: prefix + `video_download ${result.url}|360|document|${result.thumbnail}|${result.title}` },
                    { title: '```720p Document```', rowId: prefix + `video_download ${result.url}|720|document|${result.thumbnail}|${result.title}` },
                    { title: '```1080p Document```', rowId: prefix + `video_download ${result.url}|1080|document|${result.thumbnail}|${result.title}` }
                ]
            }    
        ]
            ...
            sections: [
                {
                    title: "Video Type 📽️",
                    rows: [
                        { title: "144p Video", description: "144p quality download", id: prefix + `video_download ${result.url}|144|video` },
                        { title: "240p Video", description: "240p quality download", id: prefix + `video_download ${result.url}|240|video` },
                        { title: "360p Video", description: "360p quality download", id: prefix + `video_download ${result.url}|360|video` },
                        { title: "720p Video", description: "720p quality download", id: prefix + `video_download ${result.url}|720|video` },
                        { title: "1080p Video", description: "1080p quality download", id: prefix + `video_download ${result.url}|1080|video` }
                    ]
                },
                {
                    title: "Document Type 📁",
                    rows: [
                        { title: "144p Document", description: "144p quality download", id: prefix + `video_download ${result.url}|144|document|${result.thumbnail}|${result.title}` },
                        { title: "240p Document", description: "240p quality download", id: prefix + `video_download ${result.url}|240|document|${result.thumbnail}|${result.title}` },
                        { title: "360p Document", description: "360p quality download", id: prefix + `video_download ${result.url}|360|document|${result.thumbnail}|${result.title}` },
                        { title: "720p Document", description: "720p quality download", id: prefix + `video_download ${result.url}|720|document|${result.thumbnail}|${result.title}` },
                        { title: "1080p Document", description: "1080p quality download", id: prefix + `video_download ${result.url}|1080|document|${result.thumbnail}|${result.title}` }
                    ]
                }
            ]
        }
...
// Command to handle video/document download
cmd({
    pattern: "video_download",
    desc: "Download video or document based on quality and type",
    category: "download",
    filename: __filename
}, async(conn, mek, m, { from, args, reply }) => {
    try {
        const [url, quality, type, thumbnail, title] = args[0].split('|');
        const apiUrl = `https://tharuzz-ofc-api-v3.vercel.app/api/ytdl/yt?url=${encodeURIComponent(url)}&format=${quality}`;
        const response = await fetch(apiUrl);
        const data = await response.json();
        ...
        await reply('*Error downloading media!*');
        console.error(e);
    }
})
